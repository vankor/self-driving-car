...
const double v_error_weight = 1;
const double delta_error_weight = 7;
const double a_weight = 7;
const double steer_speed_error_weight = 500;
const double delta_diff_error_weight = 200;
const double a_diff_error_weight = 10;


double reference_velocity = 70;
size_t x_start = 0;
 ...
 public:
  ...
  typedef CPPAD_TESTVECTOR(AD<double>) ADvector;
    ...
    void initCostFunction(ADvector& fg, const ADvector& vars){
          ...
          for (int i = 0; i < N; i++) {
            fg[0] += cte_error_weight * CppAD::pow(vars[cte_start + i], 2);
            fg[0] += epsi_error_weight * CppAD::pow(vars[epsi_start + i], 2);
          ...
          for (int i = 0; i < N - 2; i++) {
            fg[0] += delta_diff_error_weight * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
            fg[0] += a_diff_error_weight * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
          }
    }
  ...
  for (int i = 0; i < n_vars; i++) {
    ...
    Dvector constraints_upperbound(n_constraints);
      ...
      // Set the initial variable values
        ...
        // Steering upper and lower limits of delta are set to -25 and 25
